...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (updateError) {
      setError(t('auth.reset_update_error'))
      return
    }
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden py-12">
      ...
      >
        ...
        <div className="bg-card rounded-b-2xl p-6 shadow-lg border border-border/50 border-t-0">
          <h2 className="text-xl font-bold text-foreground mb-2 font-arabic">{t('auth.reset_password_title')}</h2>
          <p className="text-sm text-muted-foreground mb-6 font-arabic">{t('auth.reset_password_subtitle')}</p>

          {!session && !success ? (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-destructive/10 text-destructive text-sm font-arabic text-center">
                {t('auth.reset_link_invalid')}
              </div>
              <Link to="/forgot-password" className="block text-center text-sm font-semibold text-primary hover:underline font-arabic">
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5 font-arabic">
                    {t('student_profile.new_password')}
                  </label>
                  <input
                    type="password"
                    value={password}
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5 font-arabic">
                    {t('student_profile.confirm_new_password')}
                  </label>
                  <input
                    type="password"
                ...
                >
                  {saving ? '...' : t('auth.update_password_btn')}
                </button>
              </form>
            </>
          )}
